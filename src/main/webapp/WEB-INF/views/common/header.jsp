...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	...
	<a class="navbar-brand" href="${pageContext.request.contextPath}">
		...
		<sec:authorize access="isAnonymous()">
			<a href="${pageContext.request.contextPath}/ch17/loginForm" class="btn btn-success btm-sm">로그인</a>
		</sec:authorize>
		<sec:authorize access="isAuthenticated()">
			...
			<a href="${pageContext.request.contextPath}/logout" class="btn btn-danger btm-sm">로그아웃</a>--%>

			<%--사이트간 요청 위조 방지 설정이 활성화되어 있을 경우 POST 방식으로만 요청 가능--%>
			<form method="post" action="${pageContext.request.contextPath}/logout">
				<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
				<button type="submit" class="btn btn-danger btm-sm">로그아웃</button>
			</form>
		</sec:authorize>
		</div>
	</div>
